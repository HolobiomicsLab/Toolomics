...
async def test_csv_operations():
    ...
    async with Client("http://localhost:5001/mcp") as client:
        ...
        result = await client.call_tool(
            "query_csv", {"name": "employees", "query": "salary > 70000"}
        )
        query_data = json.loads(result[0].text) if result else {}
        print(
            f"🔍 Query result (salary > 70000): {query_data.get('result_count', 0)} matches"
        )
        ...
        result = await client.call_tool(
            "get_csv_data", {"name": "employees", "limit": 2, "offset": 1}
        )
        page_data = json.loads(result[0].text) if result else {}
        print(
            f"📄 Pagination test (limit=2, offset=1): {page_data.get('returned_rows', 0)} rows returned"
        )
        ...
        # Try invalid query
        result = await client.call_tool(
            "query_csv", {"name": "employees", "query": "invalid_column > 100"}
        )
        ...
        # Delete a row
        result = await client.call_tool(
            "delete_csv_row", {"name": "employees", "index": 0}
        )
