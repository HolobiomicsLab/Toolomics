...
@mcp.tool
@return_as_dict
def initialize_pdf_navigation(filename: str, session_id: Optional[str] = None) -> Dict[str, Any]:
    ...
    try:
        ...
        if session_id is None:
            session_id = hashlib.md5(f"{filename}_{hash(str(pages_content))}".encode()).hexdigest()[:12]
...
def navigate_to_page(session_id: str, page_number: int) -> Dict[str, Any]:
    ...
    try:
        ...
        return CommandResult(
            status="error",
            stderr=f"Failed to navigate to page: {str(e)}",
            exit_code=1
        )
...
def get_current_page(session_id: str) -> Dict[str, Any]:
    ...
    try:
        ...
        return CommandResult(
            status="error",
            stderr=f"Failed to get current page: {str(e)}",
            exit_code=1
        )
...
def navigate_previous_page(session_id: str) -> Dict[str, Any]:
    ...
    try:
        ...
        if prev_page < 1:
            return CommandResult(
                status="error",
                stderr="Already at first page (1)",
                exit_code=1
            )
...
def add_bookmark(session_id: str, page_number: Optional[int] = None) -> Dict[str, Any]:
    ...
    try:
        ...
        nav_state = _navigation_state[session_id]
        bookmark_page = page_number if page_number is not None else nav_state.current_page
        ...
        return CommandResult(
            status="error",
            stderr=f"Failed to add bookmark: {str(e)}",
            exit_code=1
        )

@mcp.tool
@return_as_dict
def search_in_current_session(session_id: str, query: str, context_pages: int = 1) -> Dict[str, Any]:
...
@mcp.tool
@return_as_dict
def rag_query_current_session(session_id: str, query: str, top_k: int = 3, search_scope: str = "all") -> Dict[str, Any]:
    ...
    try:
        ...
        if search_scope == "current":
            ...
            pages_to_search = nav_state.bookmarks if nav_state.bookmarks else [nav_state.current_page]
        else:  # "all"
        ...
        if not chunks:
            return CommandResult(
                status="error",
                stderr="No content found in search scope",
                exit_code=1
            )
        ...
        return CommandResult(
            status="error",
            stderr=f"Failed to perform RAG query: {str(e)}",
            exit_code=1
        )
        ...
        return CommandResult(
            ...
            stdout=json.dumps({
                ...
                "navigation_info": {
                    ...
                    "progress_percentage": round((nav_state.current_page / nav_state.total_pages) * 100, 1)
                }
            })
        )
